...
export class RenderSystem {
    ...
    regionToZoomedCamera(
        box2d: [number, number, number, number],
        overviewCameraPos: THREE.Vector3,
        ...
        // Find 3D world center of the region via raycast
        const centerProjection = this.project2DTo3D(cx, cy, overviewCameraPos, overviewLookAt);

        let worldCenter: THREE.Vector3;
        if (centerProjection) {
            worldCenter = new THREE.Vector3(centerProjection.point.x, centerProjection.point.y, 0);
        } else {
            // Fallback: estimate from camera geometry
            const fovRad = THREE.MathUtils.degToRad(this.camera.fov / 2);
            const halfH = overviewCameraPos.z * Math.tan(fovRad);
            const halfW = halfH * this.camera.aspect;
            const ndcX = (cx / 1000) * 2 - 1;
            const ndcY = -((cy / 1000) * 2 - 1);
            worldCenter = new THREE.Vector3(
                overviewCameraPos.x + ndcX * halfW,
                overviewCameraPos.y + ndcY * halfH,
                0
            );
        }

        // Calculate Z height so camera frustum covers the region with padding
        const regionWidth = (xmax - xmin) / 1000;
        const regionHeight = (ymax - ymin) / 1000;
        const fovRad = THREE.MathUtils.degToRad(this.camera.fov / 2);
        const overviewHalfH = overviewCameraPos.z * Math.tan(fovRad);
        const overviewHalfW = overviewHalfH * this.camera.aspect;
        const regionWorldH = regionHeight * overviewHalfH * 2;
        const regionWorldW = regionWidth * overviewHalfW * 2;
        const requiredZH = (regionWorldH * padding) / (2 * Math.tan(fovRad));
        const requiredZW = (regionWorldW * padding) / (2 * Math.tan(fovRad) * this.camera.aspect);
        const zoomZ = Math.max(0.5, Math.min(1.2, Math.max(requiredZH, requiredZW)));

        return {
            position: new THREE.Vector3(worldCenter.x, worldCenter.y - 0.005, zoomZ),
            target: new THREE.Vector3(worldCenter.x, worldCenter.y, 0),
        };
    }
