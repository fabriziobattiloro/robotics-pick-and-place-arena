...
import { BoxSelect, ChevronDown, Download, FastForward, Grab, History, Info, Loader2, MousePointer2, PlayCircle, RotateCcw, Scan, Send, Settings2, Thermometer, X } from 'lucide-react';
import { useState } from 'react';
import { LogOverlay } from '../App';
import { MODEL_OPTIONS, type Provider } from '../models';
...
interface UnifiedSidebarProps {
  isOpen: boolean;
  onClose: () => void;
  onSend: (prompt: string, type: DetectType, temperature: number, enableThinking: boolean, modelId: string, enableSpatialSegmentation: boolean, optimisedPrompts: boolean) => void;
  onRunAll: (prompt: string, type: DetectType, temperature: number, enableThinking: boolean, enableSpatialSegmentation: boolean, optimisedPrompts: boolean) => void;
  onDownloadResults: () => void;
  onPickup: () => void;
  isLoading: boolean;
  runAllLoading?: boolean;
  runAllProgress?: { current: number; total: number; modelId: string } | null;
  hasRunAllResults?: boolean;
...
export function UnifiedSidebar({
  ...
  const [enableSpatialSegmentation, setEnableSpatialSegmentation] = useState(false);
  const [optimisedPrompts, setOptimisedPrompts] = useState(false);
  ...
  const isInvalidConfig = isMasks && currentModel?.supportsSegmentation === false;
            ...
            <div className="relative">
              ...
              >
                {groupedModels.map(([provider, models]) => (
                  <optgroup key={provider} label={PROVIDER_LABELS[provider as Provider] || provider}>
                    {models.map(m => (
        ...
        <section className="space-y-1.5">
          <div className={`p-1.5 rounded-2xl flex border ${selectorBg}`}>
              {(['2D bounding boxes', 'Segmentation masks', 'Points'] as DetectType[]).map((t) => {
                ...
                const isItemDisabled = isSeg && currentModel?.supportsSegmentation === false;
          ...
          {isInvalidConfig && (
             <div className={`p-3 rounded-xl border text-[10px] leading-relaxed animate-in slide-in-from-top-1 fade-in duration-200 ${isDarkMode ? 'bg-amber-500/10 border-amber-500/20 text-amber-200' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>
                <p>
                  <span className="font-bold">Image segmentation:</span> Segmentation masks (returning pixel-level masks for objects) are only supported by <strong>gemini-robotics-er-1.5-preview</strong>. Please select that model or choose a different detection type.
                </p>
             </div>
          )}
        </section>
        ...
        <section className="space-y-2">
          ...
          {showSettings && (
            <div className={`flex items-center justify-between gap-4 px-1 py-1 animate-in slide-in-from-top-2 fade-in duration-200 ${isDarkMode ? 'bg-slate-800/20 rounded-xl' : 'bg-slate-50/50 rounded-xl'}`}>
               ...
               <div className="flex items-center gap-4 pt-4 p-2">
                   <div className="flex items-center gap-2" title="Thinking improves the capabilities of the model to reason through tasks, but may produce less desirable results for simple locating tasks. For simple tasks, disable thinking for improved speed and likely better results.">
                       <input
                         type="checkbox"
                         id="thinking-toggle"
                   ...
                   <div className="flex items-center gap-2" title="Two-pass detection: first identifies regions of interest from an overview, then zooms into each region for fine-grained detection. Improves accuracy on cluttered scenes.">
                       <input
                         type="checkbox"
                         id="spatial-seg-toggle"
                         checked={enableSpatialSegmentation}
                         onChange={(e) => setEnableSpatialSegmentation(e.target.checked)}
                         className="w-3.5 h-3.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                       />
                       <label htmlFor="spatial-seg-toggle" className={`text-[9px] font-bold uppercase tracking-widest cursor-pointer select-none ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                          Two-Pass
                       </label>
                   </div>
                   <div className="flex items-center gap-2" title="Use optimised prompts with detailed instructions for improved detection accuracy across all providers.">
                       <input
                         type="checkbox"
                         id="optimised-prompts-toggle"
                         checked={optimisedPrompts}
                         onChange={(e) => setOptimisedPrompts(e.target.checked)}
                         className="w-3.5 h-3.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                       />
                       <label htmlFor="optimised-prompts-toggle" className={`text-[9px] font-bold uppercase tracking-widest cursor-pointer select-none ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                          Optimised
                       </label>
                   </div>
               </div>
          ...
          <div className="flex gap-3">
            <button 
                onClick={() => onSend(prompt, type, temperature, enableThinking, modelId, enableSpatialSegmentation, optimisedPrompts)}
          ...
          <div className="flex gap-3">
            <button
                onClick={() => onRunAll(prompt, type, temperature, enableThinking, enableSpatialSegmentation, optimisedPrompts)}
                disabled={isLoading || runAllLoading || !prompt.trim()}
                title="Run detection with all available models and capture results"
                className={`flex-1 py-3 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 transition-all ${
                  isLoading || runAllLoading
                    ? 'bg-slate-200 text-slate-400 cursor-not-allowed dark:bg-slate-800 dark:text-slate-600'
                    : 'bg-amber-500 text-white hover:bg-amber-600 shadow-lg active:scale-[0.98] dark:bg-amber-600 dark:hover:bg-amber-700'
                }`}
            >
                {runAllLoading ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    <span>{runAllProgress ? `${runAllProgress.current}/${runAllProgress.total} ${runAllProgress.modelId.split('-').slice(0, 2).join('-')}` : 'Running...'}</span>
                  </>
                ) : (
                  <>
            ...
            {hasRunAllResults && (
              <button
                  ...
                  title="Download all results as a zip file"
                  className={`py-3 px-4 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 transition-all shadow-lg active:scale-[0.98] ${
                      ...
                      {log.result === null ? (
                        ...
                        <div className="flex items-center justify-between mt-2 gap-2">
                           ...
                           <button 
                             onClick={(e) => {
                               e.stopPropagation();
                               onSend(log.prompt, log.type as DetectType, temperature, enableThinking, modelId, enableSpatialSegmentation, optimisedPrompts);
                             }}
