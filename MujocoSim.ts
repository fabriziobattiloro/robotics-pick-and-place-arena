...
export interface SavedCubeConfigEntry {
    name: string;
...
export interface SavedCubeConfig {
    cubes: SavedCubeConfigEntry[];
}
...
export class MujocoSim {
    ...
    async init(robotId = 'franka_emika_panda', sceneFile = 'scene.xml', onProgress?: (msg: string) => void) {
        tlog('SIM', 'Initializing simulation', { robotId, sceneFile });
        ...
        if (this.mjModel) {
            tlog('SIM', 'MuJoCo model loaded', { bodies: this.mjModel.nbody, geoms: this.mjModel.ngeom, actuators: this.mjModel.nu });
            ...
            tlog('SIM', 'Simulation ready — starting render loop');
            this.startLoop();
        }
    }
    ...
    private getCubeBodyInfo(): Array<{ bodyId: number; name: string; qposAdr: number }> {
        if (!this.mjModel) return [];
        const cubes: Array<{ bodyId: number; name: string; qposAdr: number }> = [];
        for (let i = 0; i < this.mjModel.nbody; i++) {
            ...
            if (jntIdVal < 0) continue;
            cubes.push({ bodyId: i, name, qposAdr: this.mjModel.jnt_qposadr[jntIdVal] });
        }
        return cubes;
    }

    private setBodyColor(bodyId: number, rgba: [number, number, number, number]) {
        if (!this.mjModel) return;
        for (let g = 0; g < this.mjModel.ngeom; g++) {
            if (this.mjModel.geom_bodyid[g] !== bodyId) continue;
            const offset = g * 4;
            this.mjModel.geom_rgba[offset] = rgba[0];
            this.mjModel.geom_rgba[offset + 1] = rgba[1];
            this.mjModel.geom_rgba[offset + 2] = rgba[2];
            this.mjModel.geom_rgba[offset + 3] = rgba[3];
        }
    }

    private getBodyColor(bodyId: number): [number, number, number, number] {
        if (!this.mjModel) return [1, 1, 1, 1];
        for (let g = 0; g < this.mjModel.ngeom; g++) {
            if (this.mjModel.geom_bodyid[g] !== bodyId) continue;
            const offset = g * 4;
            return [
                this.mjModel.geom_rgba[offset] ?? 1,
                this.mjModel.geom_rgba[offset + 1] ?? 1,
                this.mjModel.geom_rgba[offset + 2] ?? 1,
                this.mjModel.geom_rgba[offset + 3] ?? 1,
            ];
        }
        return [1, 1, 1, 1];
    }

    captureCubeConfig(): SavedCubeConfig {
        if (!this.mjModel || !this.mjData) return { cubes: [] };
        const cubes = this.getCubeBodyInfo().map((cube) => {
            const qp = cube.qposAdr;
            return {
                name: cube.name,
                position: [
                    this.mjData!.qpos[qp],
                    this.mjData!.qpos[qp + 1],
                    this.mjData!.qpos[qp + 2],
                ] as [number, number, number],
                quaternion: [
                    this.mjData!.qpos[qp + 3],
                    this.mjData!.qpos[qp + 4],
                    this.mjData!.qpos[qp + 5],
                    this.mjData!.qpos[qp + 6],
                ] as [number, number, number, number],
                rgba: this.getBodyColor(cube.bodyId),
            };
        });
        return { cubes };
    }

    applyCubeConfig(config: SavedCubeConfig): boolean {
        if (!this.mjModel || !this.mjData || !config?.cubes?.length) return false;
        const byName = new Map(config.cubes.map((c) => [c.name, c]));
        let applied = 0;
        for (const cube of this.getCubeBodyInfo()) {
            const saved = byName.get(cube.name);
            if (!saved) continue;
            const qp = cube.qposAdr;
            this.mjData.qpos[qp] = saved.position[0];
            this.mjData.qpos[qp + 1] = saved.position[1];
            this.mjData.qpos[qp + 2] = saved.position[2];
            this.mjData.qpos[qp + 3] = saved.quaternion[0];
            this.mjData.qpos[qp + 4] = saved.quaternion[1];
            this.mjData.qpos[qp + 5] = saved.quaternion[2];
            this.mjData.qpos[qp + 6] = saved.quaternion[3];
            this.setBodyColor(cube.bodyId, saved.rgba);
            applied++;
        }
        if (applied > 0) {
            this.mujoco.mj_forward(this.mjModel, this.mjData);
    ...
    pickupItems(positions: THREE.Vector3[], markerIds: number[], onFinished?: () => void) {
        if (this.sequenceAnimator && this.mjData) {
            tlog('SIM', `Starting pickup — ${positions.length} items`, { positions: positions.map(p => `(${p.x.toFixed(3)}, ${p.y.toFixed(3)}, ${p.z.toFixed(3)})`) });
            this.ikSys.syncToSite(this.mjData);
    ...
    reset() {
        if (!this.mjModel || !this.mjData) return;
        tlog('SIM', 'Resetting simulation — randomizing cubes');
