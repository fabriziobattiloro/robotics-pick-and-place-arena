...
export class SequenceAnimator {
    ...
    start(ikTarget: THREE.Object3D, mjData: MujocoData, ikSystem: IkSystem, 
        ...
        const totalItems = this.useLivePosition ? this.cubeIds.length : this.targetPositions.length;
        tlog('ANIMATOR', 'Sequence started', { mode: this.useLivePosition ? 'live' : 'blind', totalItems, stacking: this.isStacking });
    ...
    update(dt: number, ikTarget: THREE.Object3D, mjData: MujocoData, gripperId: number, ikSystem: IkSystem) {
        ...
        if (p >= 1.0) {
            this.step++;
            const stepName = this.names[this.step] ?? `Done (step ${this.step})`;
            tlog('ANIMATOR', `Step ${this.step}: ${stepName}`, { cube: this.curCubeIdx + 1, gripper: this.gripperVal === 0 ? 'closed' : 'open' });
            this.prepareStep(ikTarget, mjData, ikSystem);
        }
    }
    ...
    prepareStep(ikTarget: THREE.Object3D, mjData: MujocoData, ikSystem: IkSystem) {
         ...
         switch(this.step) {
            ...
            case 14: // Check next or Return Home
                this.droppedCount++;
                tlog('OK', `Item ${this.curCubeIdx + 1} picked & placed`, { droppedTotal: this.droppedCount });
                ...
                tlog('ANIMATOR', 'All items placed — returning home');
                this.duration = 2.0;
         ...
         if (!useExplicitJoints) {
             ...
             if (sol) {
                 ...
                 tlog('ERR', `IK solve failed for step ${this.step}`);
             }
         }
    }
}
